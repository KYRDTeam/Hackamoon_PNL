...
const getPoolPNLOfWallet = async (walletAddress: any, poolAddress: any) => {
  try {
    ...
    // TODO: for quick prototype, just calculate deposit amount (ignore listTransaction?.withdraw for now)
...
const getListDemoWallet = async () => {
  try {
    ...
    console.log("listData", listData.slice(530, 550));
  } catch (error: any) {
...
const getPriceOfToken = async (contractAddress: any, from: any, to: any) => {
  try {
    const res = await coinGekoAxios?.get(
      `/contract/${String(
        contractAddress
      ).toLowerCase()}/market_chart/range?vs_currency=usd&from=${from}&to=${to}`
    );
    const listPrice = res?.data?.prices;
    return listPrice?.[0]?.[1];
  } catch (error: any) {
    console.log("getPriceOfToken error:", error);
  }
};
